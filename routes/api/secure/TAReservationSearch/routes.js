...
const Process = async (params) => {
    ...
    let db = new sqldb()
    await db.connect()
    const dbResult = await db.TA_SearchReservationStatus(params)
...
const getTASearchReservationStatus = (req, res, next) => {
...
router.post('/reservation/search', getTASearchReservationStatus)

...
const Process = async (params) => {
    ...
    let db = new sqldb()
    await db.connect()
    let dbResult = await db.Sap_GetReservationItem(params)
...
const getTASearchReservationItems = (req, res, next) => {
...
router.post('/reservation/items/search', getTASearchReservationItems)

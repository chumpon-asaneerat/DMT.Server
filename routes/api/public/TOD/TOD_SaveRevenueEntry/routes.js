...
//#region Implement - TOD_SaveRevenueEntry

api.TOD_SaveRevenueEntry = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.TOD_SaveRevenueEntry(params)
    }
    ...
    static entry(req, res) {
        let ref = api.TOD_SaveRevenueEntry
        let db = new sqldb()
...
router.all('/', api.TOD_SaveRevenueEntry.entry)

const init_routes = (svr) => {
    svr.route('/api/TOD/RevenueEntry/save', router);
};

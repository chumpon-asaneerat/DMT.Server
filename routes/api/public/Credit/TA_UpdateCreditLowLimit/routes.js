...
//#region Implement - TA_UpdateCreditLowLimit

api.TA_UpdateCreditLowLimit = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.TA_UpdateCreditLowLimit(params)
    }
    ...
    static entry(req, res) {
        let ref = api.TA_UpdateCreditLowLimit
        let db = new sqldb()
...
router.all('/', api.TA_UpdateCreditLowLimit.entry)

const init_routes = (svr) => {
    svr.route('/api/TA/CreditLowLimit/save', router);
};

...
//#region Implement - TA_getSelltoInterface

api.TA_getSelltoInterface = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.TA_getSelltoInterface(params)
    }
    ...
    static entry(req, res) {
        let ref = api.TA_getSelltoInterface
        let db = new sqldb()
...
router.all('/', api.TA_getSelltoInterface.entry)

const init_routes = (svr) => {
    svr.route('/api/account/sap/couponsoldlist', router);
};

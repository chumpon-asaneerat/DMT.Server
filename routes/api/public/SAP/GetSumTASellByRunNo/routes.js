...
//#region Implement - GetSumTASellByRunNo

api.GetSumTASellByRunNo = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.GetSumTASellByRunNo(params)
    }
    ...
    static entry(req, res) {
        let ref = api.GetSumTASellByRunNo
        let db = new sqldb()
...
router.all('/', api.GetSumTASellByRunNo.entry)

const init_routes = (svr) => {
    svr.route('/api/account/sap/get/arsumcoupon', router);
};

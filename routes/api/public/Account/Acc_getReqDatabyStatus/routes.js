...
//#region Implement - Acc_getReqDatabyStatus

api.Acc_getReqDatabyStatus = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_getReqDatabyStatus(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_getReqDatabyStatus
        let db = new sqldb()
...
router.all('/', api.Acc_getReqDatabyStatus.entry)

const init_routes = (svr) => {
    svr.route('/api/account/request/getlist', router);
};

...
//#region Implement - TA_SaveReqExchangeDetial

api.TA_SaveReqExchangeDetial = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.TA_SaveReqExchangeDetial(params)
    }
    ...
    static entry(req, res) {
        let ref = api.TA_SaveReqExchangeDetial
        let db = new sqldb()
...
router.all('/', api.TA_SaveReqExchangeDetial.entry)

const init_routes = (svr) => {
    svr.route('/api/account/request/savedetail', router);
};

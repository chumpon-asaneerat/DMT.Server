...
//#region Implement - TA_GetUpdateReqData

api.TA_GetUpdateReqData = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.TA_GetUpdateReqData(params)
    }
    ...
    static entry(req, res) {
        let ref = api.TA_GetUpdateReqData
        let db = new sqldb()
...
router.all('/', api.TA_GetUpdateReqData.entry)

const init_routes = (svr) => {
    svr.route('/api/account/request/getapprove', router);
};

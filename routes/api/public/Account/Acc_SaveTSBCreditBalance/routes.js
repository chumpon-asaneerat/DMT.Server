...
//#region Implement - Acc_SaveTSBCreditBalance

api.Acc_SaveTSBCreditBalance = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_SaveTSBCreditBalance(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_SaveTSBCreditBalance
        let db = new sqldb()
...
router.all('/', api.Acc_SaveTSBCreditBalance.entry)

const init_routes = (svr) => {
    svr.route('/api/account/tsbcredit/save', router);
};

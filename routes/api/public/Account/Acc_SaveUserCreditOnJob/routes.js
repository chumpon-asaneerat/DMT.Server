...
//#region Implement - Acc_SaveUserCreditOnJob

api.Acc_SaveUserCreditOnJob = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_SaveUserCreditOnJob(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_SaveUserCreditOnJob
        let db = new sqldb()
...
router.all('/', api.Acc_SaveUserCreditOnJob.entry)

const init_routes = (svr) => {
    svr.route('/api/account/UserCredit/save', router);
};

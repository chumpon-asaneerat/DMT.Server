...
//#region Implement - TA_GetAccAppReqDetail

api.TA_GetAccAppReqDetail = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.TA_GetAccAppReqDetail(params)
    }
    ...
    static entry(req, res) {
        let ref = api.TA_GetAccAppReqDetail
        let db = new sqldb()
...
router.all('/', api.TA_GetAccAppReqDetail.entry)

const init_routes = (svr) => {
    svr.route('/api/account/request/getapprovedetail', router);
};

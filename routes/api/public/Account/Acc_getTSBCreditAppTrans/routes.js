...
//#region Implement - Acc_getTSBCreditAppTrans

api.Acc_getTSBCreditAppTrans = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_getTSBCreditAppTrans(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_getTSBCreditAppTrans
        let db = new sqldb()
...
router.all('/', api.Acc_getTSBCreditAppTrans.entry)

const init_routes = (svr) => {
    svr.route('/api/account/appcredit/translist', router);
};

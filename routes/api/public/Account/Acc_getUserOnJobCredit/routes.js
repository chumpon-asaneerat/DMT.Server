...
//#region Implement - Acc_getUserOnJobCredit

api.Acc_getUserOnJobCredit = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_getUserOnJobCredit(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_getUserOnJobCredit
        let db = new sqldb()
...
router.all('/', api.Acc_getUserOnJobCredit.entry)

const init_routes = (svr) => {
    svr.route('/api/account/usercredit/search', router);
};

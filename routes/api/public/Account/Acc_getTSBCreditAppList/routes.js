...
//#region Implement - Acc_getTSBCreditAppList

api.Acc_getTSBCreditAppList = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_getTSBCreditAppList(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_getTSBCreditAppList
        let db = new sqldb()
...
router.all('/', api.Acc_getTSBCreditAppList.entry)

const init_routes = (svr) => {
    svr.route('/api/account/appcredit/list', router);
};

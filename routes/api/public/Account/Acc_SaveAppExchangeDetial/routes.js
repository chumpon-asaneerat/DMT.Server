...
//#region Implement - Acc_SaveAppExchangeDetial

api.Acc_SaveAppExchangeDetial = class {
    static prepare(req, res) {
        let params = WebServer.parseReq(req).data
        return params
    }
    static async call(db, params) { 
        return db.Acc_SaveAppExchangeDetial(params)
    }
    ...
    static entry(req, res) {
        let ref = api.Acc_SaveAppExchangeDetial
        let db = new sqldb()
...
router.all('/', api.Acc_SaveAppExchangeDetial.entry)

const init_routes = (svr) => {
    svr.route('/api/account/request/approve', router);
};
